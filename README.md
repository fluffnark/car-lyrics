...
The catalog uses the channel's public Atom feed, which contains only recent uploads (currently about 15). It caches the list on the phone so browsing survives a temporary feed failure; playback still needs internet. Saved songs keep their titles and video IDs locally even after they leave the recent feed. A larger searchable catalog and album or playlist workflow are future work. Videos must allow embedding and may include ads. Morphe is no longer in the playback path because capturing its app window required phone consent every session.
...
The APK is at `app/build/outputs/apk/debug/app-debug.apk` and the AAB is at `app/build/outputs/bundle/debug/app-debug.aab`. Automated tests cover feed parsing, saved-song persistence, browse and pagination, selecting a song, playback actions, error retry, and hardware Back. The phone companion screen was rendered and visually reviewed. The connected Pixel 7a fetched and cached 15 live Sing King videos. **Car display playback, YouTube WebView rendering, audio routing, and Mazda knob behavior still require a parked real-car test.**

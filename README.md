...
The service declares the POI category, app icon, `template` capability, and Google Android Auto host certificates, including the SHA-256 certificate measured from the Android Auto APK on this Pixel. The phone's Android Auto developer mode and Unknown sources setting are already enabled. The Desktop Head Unit launches Car Lyrics and accepts rotary/d-pad navigation into the player template. A fresh run reports a valid 770×700 presentation surface and hardware WebView. The Sing King test upload returned YouTube error 150 (owner-disabled embedding), so the renderer is working but that upload cannot prove end-to-end playback; a playable upload or the Morphe mirror path is still needed to validate audio/video.
